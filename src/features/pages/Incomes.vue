...
import {useIncomes} from "../entities/hooks/useIncomes";
...
// Форматирование текущей и вчерашней даты
const today = format(new Date(), "yyyy-MM-dd");
const yesterday = format(subDays(new Date(), 1), "yyyy-MM-dd");

// Получение данных с хука
const {data, isFetching, setPage, setLimit, setDateRange, refetch} = useIncomes({
  config: {
    dateFrom: yesterday,
    dateTo: today,
...
const applyFilters = () => {
  ...
  setPage(1);
  refetch(); // Принудительный refetch
};
...
<template>
    ...
    <div v-if="viewMode === 'table' && !isFetching">
      <DataTable :value="filteredData" class="p-datatable-sm" responsiveLayout="scroll">
        ...
        <Column field="date_close" header="Дата закрытия" sortable></Column>
