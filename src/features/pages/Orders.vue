...
import { useOrders } from "../entities/hooks/useOrders";
...
const { data, isFetching, setPage, setLimit, setDateRange, refetch } = useOrders({
  config: {
    dateFrom: yesterday,
    dateTo: today,
...
<template>
    ...
    <div v-if="viewMode === 'table' && !isFetching">
      <DataTable :value="filteredData" class="p-datatable-sm" responsiveLayout="scroll">
        ...
        <Column field="oblast" header="Область" sortable></Column>
        ...
        <Column field="is_cancel" header="Отменён" sortable>
          <template #body="{ data }">
            {{ data.is_cancel ? "Да" : "Нет" }}
          </template>
        </Column>
        <Column field="cancel_dt" header="Дата отмены" sortable></Column>
      </DataTable>
    </div>
